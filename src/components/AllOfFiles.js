...
export default function AllOfFiles() {
...
//     try {
//       const deleteUrl =
//         API_URL + '/users/removeFile/' + ID + '/' + encodeURIComponent(selectedFileUrl);
    ...
    <div>
      <div className="font-medium text-neutral-300 mb-0.5 border-2 p-3 m-0.5 flex justify-between colors2 rounded-lg">
        {/* <span className="font-bold text-lg">
          <span className="font-extralight">מסמכים של:</span> {client?.name}
        </span> */}
      </div>
      <div className="overflow-y-scroll max-h-[70vh] flex flex-wrap m-1 p-3 rounded-lg custom-shadow border-blue-200 custom-shadow justify-between colors2">
        {/* {fileClient.map((file, index) => (
          <div
            className="flex flex-wrap w-full sm:w-auto custom-shadow rounded-lg p-4 shadow-md mb-4"
            key={index}
          >
            {getDisplayURL(file)}
            <button
